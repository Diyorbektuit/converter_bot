...
from utils.archive import create_zip
import os
...
# zip
@router.message(lambda message: message.text == "📦 ZIP formatida arxivlash")
...
async def handle_file(message: Message, state: FSMContext):
    ...
    if message.text == "⬅️ Orqaga qaytish":
        ...
        archive_name = f"downloads/archive/zip-{user_id}-{str(today.month)}-{str(today.day)}-{str(today.minute)}-{str(today.second)}.zip"
        
        await message.answer("Tayyorlanmoqda ...")
        
        zip_path = create_zip(file_list, archive_name)
        if zip_path is None:
            ...
            return await message.answer("Xatolik yuz berdi boshqattan urinib ko'ring")
        send_file = FSInputFile(path=zip_path)
